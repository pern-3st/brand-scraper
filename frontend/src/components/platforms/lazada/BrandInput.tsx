...
interface LazadaBrandInputProps {
  shopUrl: string;
  maxProducts: number;
...
export default function LazadaBrandInput({
  shopUrl,
  maxProducts,
  onShopUrlChange,
  onMaxProductsChange,
}: LazadaBrandInputProps) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          id="lazada-shop-url"
          type="url"
          placeholder="https://www.lazada.sg/shop/lacoste/"
          value={shopUrl}
        ...
        <p className="mt-1.5 text-xs text-muted-fg">
          Paste the storefront URL, e.g. https://www.lazada.sg/shop/lacoste/
        </p>
      </div>
      <div>
        <label
          htmlFor="lazada-max-products"
          className="block text-sm font-medium text-foreground/70 mb-1.5"
        >
          Max products
        </label>
        <input
          id="lazada-max-products"
          type="number"
          inputMode="numeric"
          min={MIN_PRODUCTS}
          max={MAX_PRODUCTS}
